...
function mkvKeyframes(url) {
  return new Promise((resolve, reject) => {
    ...
    decoder.parseEbmlIDs(url, [mkv.Schema.byName.Cues, mkv.Schema.byName.Tracks], function (err, doc) {
      ...
      if (videoTrackIdx === -1) return reject(new Error('no video tracks found'))

      // Go through CuePoint(s) and filter out the ones which are from the video track
      var cues = atPath(doc, "Segment", "Cues");
      if (!(cues && cues.children && cues.children.length)) return reject(new Error("no cues found in doc -> Segment -> Cues"));

      cues = cues.children.filter(function (x) { return x._name === "CuePoint" })

      if (!cues.length) return reject(new Error("no CuePoints"));
      ...
      frames = frames.map(f => Number((f.pts / 1000).toFixed(3)))
      resolve(frames)
